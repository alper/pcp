...
pub enum RobotType {
    ...
    Duration {
        vdurations: Vec<Var<VStore>>,
        variation: usize,
    }, //task duration is a variable
}
    ...
    fn clone(&self) -> Self {
        match self {
            ...
            &RobotType::Duration {
                ...
                vdurations: vdurations.iter().map(|v| v.bclone()).collect(),
                variation: variation,
            }, //task duration is a variable
        }
    }
...
impl Robot {
    ...
    pub fn get_pipeting_duration_at_rank(&self, i: usize) -> Var<VStore> {
        match self.robot_type {
            RobotType::Simple => Box::new(Identity::new(self.durations[i])),
            RobotType::Duration { ref vdurations, .. } => vdurations[i].bclone(),
        }
    }

    pub fn add_robot_duration_variables(&mut self, space: &mut FDSpace, model: &mut Model) {
        if let RobotType::Duration {
            ref mut vdurations,
            variation,
        } = self.robot_type
        {
            self.durations.iter().for_each(|duration| {
                vdurations.push(model.alloc_var(
                    &mut space.vstore,
                    IntervalSet::new(*duration as isize, (*duration + variation) as isize),
                ))
            })
        }
    }
        ...
        match self.robot_type {
            RobotType::Simple => {
                for t in 1..self.tasks.len() {
                    space.cstore.alloc(Box::new(x_greater_y(
                        self.start[t].bclone(),
                        Box::new(Addition::new(
                            self.start[t - 1].bclone(),
                            self.durations[t - 1] as isize,
                        )),
                    )));
                }
            }
            RobotType::Duration { ref vdurations, .. } => {
                for t in 1..self.tasks.len() {
                    space.cstore.alloc(Box::new(x_geq_y_plus_z(
                        self.start[t].bclone(),
                        self.start[t - 1].bclone(),
...
impl RobotScheduling {
    ...
    pub fn new_test2(
        ...
        Wduration: usize,
        Pduration: usize,
        duration_variation: usize,
    ) -> Self {
        let mut robotschel = RobotScheduling {
            robots: Robot::create_robots(
                RobotType::Duration {
                    vdurations: vec![],
                    variation: duration_variation,
                },
                num_robot,
                Lduration,
                Tduration,
    ...
    fn initialize(&mut self) {
        let time_dom = IntervalSet::new(1, self.max_time as isize);
        ...
        for (i, robot) in self.robots.iter_mut().enumerate() {
            ...
            for _ in 0..robot.tasks.len() {
                robot.start.push(
                    self.model
                        .alloc_var(&mut self.space.vstore, time_dom.clone()),
                );
            }
            ...
            for &(t, d) in robot.cumultasks.iter() {
                ...
                self.pipeting_duration
                    .push(robot.get_pipeting_duration_at_rank(d).bclone());
            }
        ...
        self.space.cstore.alloc(Box::new(XEqY::new(
            self.robots[0].start[0].bclone(),
        ...
        for (nti, ti) in inter_tasks.into_iter().enumerate() {
            for (ntj, var) in ti.into_iter().enumerate() {
                self.model.register_var(var, format!("t{}_{}", nti, ntj));
            }
        }
    ...
    pub fn solve(mut self) -> Self {
        let search = OneSolution::<_, VectorStack<_>, FDSpace>::new(
    ...
    /*  fn start_at(&self, task: usize) -> isize {
      self.start[task].read(&self.space.vstore).lower()
    } */

    /*  fn duration_at(&self, duration: usize) -> isize {
